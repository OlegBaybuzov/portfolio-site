...
const EMPTY_ARRAY_TEMPLATES = {
  projects: { title: '', description: '', image: '', github: '', demo: '', stack: [], featured: false, year: new Date().getFullYear(), status: '', tags: [] },
...
function resetShape(value) {
  ...
  if (value !== null && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([k, v]) => [k, resetShape(v)]));
  }
...
function buildPrimitive(value, path) {
  ...
  if (typeof value === 'boolean') {
    ...
    input.addEventListener('change', () => (span.textContent = input.checked ? 'Yes' : 'No'));
  ...
  const useTextarea = str.length > 60 || /description|paragraph|quote|subtitle|bio/i.test(key);
  const input = document.createElement(useTextarea ? 'textarea' : 'input');
  input.className = useTextarea ? 'field-textarea' : 'field-input';
  if (!useTextarea) input.type = key.toLowerCase().includes('email') ? 'email' : 'text';
  input.value = str;
  input.addEventListener('input', () => {
...
function itemDisplayName(item, index) {
  return item.title || item.category || item.role || item.label || item.name || `Item ${index + 1}`;
}
...
function buildNode(value, path) {
  if (Array.isArray(value)) {
    if (value.length === 0 && path.length === 1 && EMPTY_ARRAY_TEMPLATES[path[0]]) {
  ...
  if (value !== null && typeof value === 'object') return buildObject(value, path);
...
async function writeToDisk(name, json) {
  ...
  const fileHandle = await contentDir.getFileHandle(`${name}.json`, { create: true });
...
async function pickFolder() {
  if (!window.showDirectoryPicker) {
    toast('Your browser doesn\u2019t support direct file saving \u2014 use Chrome or Edge, or keep using Save to download.');
    return;
  }
  try {
    ...
    dirHandle = handle;
    document.getElementById('folderStatus').textContent = 'Connected \u2014 Save writes directly to /content';
  } catch (err) {
    if (err.name !== 'AbortError') {
      toast('Could not connect that folder \u2014 make sure it contains a /content directory');
    }
  }
}
