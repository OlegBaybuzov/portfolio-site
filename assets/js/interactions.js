...
export function initCursor() {
  ...
  document.querySelectorAll('a, button, .project-card, input, textarea').forEach((elm) => {
    elm.addEventListener('mouseenter', () => dot.classList.add('is-hover'));
    elm.addEventListener('mouseleave', () => dot.classList.remove('is-hover'));
  });
}
...
export function runHeroReveal() {
  ...
  gsap.to(lines, { yPercent: 0, duration: 0.9, ease: 'expo.out', stagger: 0.1, delay: 0.1 });

  if (nav) {
    gsap.to(nav, { opacity: 1, y: 0, duration: 0.6, ease: 'expo.out', delay: 0.15 });
    nav.classList.add('is-visible');
  }

  items.forEach((item, idx) => {
    gsap.to(item, { opacity: 1, y: 0, duration: 0.7, ease: 'expo.out', delay: 0.5 + idx * 0.12 });
  });
}
...
/* ---------- Contact form (front-end only — see README to wire a backend) ---------- */
